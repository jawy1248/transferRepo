function [Xfft,ws] = fftp(xt,ts,varargin)
% [Xfft ws] = fftp(xt,ts,DIM);
% 
...
%        of the time sequence.  No scaling is applied by fftp.
%
...
if length(ts) > 1;
    ...
    if abs(min(diff(ts)) - max(diff(ts)))/abs(max(diff(ts))) > 1e-6;
        warning(['Time samples are not equally spaced, ',...
            'min(diff(ts))=',num2str(min(diff(ts))),',  max(diff(ts))=',num2str(max(diff(ts))),...
            '. Using the average.']);
        dt=(min(diff(ts)) + max(diff(ts)))/2;
    end
else
    dt = ts;
end
